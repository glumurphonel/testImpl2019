...
contract TicketCore {
  ...
  function getTicketInfoById(uint _tId) public view ticketExists(_tId)
    returns (string memory, string memory, address, address, TicketStatus, string memory) {
    return (allTickets[_tId].title, allTickets[_tId].description,
            allTickets[_tId].tAssg, allTickets[_tId].tOwn,
            allTickets[_tId].status, getTicketStatus(allTickets[_tId].status));
  }
  ...
  function getAssignedTicketsByStatus(address _addr, TicketStatus _tStatus)
  ...
  function getOwnedTicketsByStatus(address _addr, TicketStatus _tStatus)
    public view accountExists(_addr) returns(uint[] memory) {
    uint numberOwnedTickets = allAccounts[_addr].ownedTickets.length;
    uint[] memory ids = new uint[](numberOwnedTickets);
